...
pp = PdfPages('figure_1b-final.pdf')
...
keys = [
	...
	'minigrid_figure_1b-reward_model - rollout/ep_rew_mean_extrinsic',
	'minigrid_figure_1b-reward_model - rollout/ep_rew_mean_extrinsic__MIN',
	'minigrid_figure_1b-reward_model - rollout/ep_rew_mean_extrinsic__MAX'
]
...
ax_nr = sns.lineplot(x="Step", y="minigrid_figure_1b-none - rollout/ep_rew_mean_extrinsic",
             data=ep_rew_mean_other_half, alpha=0.5, color='green', label='Without social rewards', linewidth=3)
# ax_rm = sns.lineplot(x="Step", y="minigrid_figure_1b-reward_model - rollout/ep_rew_mean_extrinsic", 
# 				  data=ep_rew_mean, alpha=0.5, color='red', label='With reward model')
ax_nr.axvline(x = 60, ymin = 0, ymax = 1, color='black', linestyle='dashed')
# plt.legend(loc=[0.55, 0.7]) # 'upper right')
plt.legend(loc=[.05,.82], prop={'size': 20})#[0, 0.7]) # 'upper right')
# plt.legend(loc=[.53,.82], prop={'size': 12})#[0, 0.7]) # 'upper right')
# plt.legend(loc=[0.55, 0.35])#[0, 0.7]) # 'upper right')
# plt.title("Learning with and without social rewards", y=1.08)
# plt.title("Learning with and without caregiver presence", y=1.08)
...
# ymin = ep_rew_mean['minigrid_figure_1b-reward_model - rollout/ep_rew_mean_extrinsic__MIN']
# ymax = ep_rew_mean['minigrid_figure_1b-reward_model - rollout/ep_rew_mean_extrinsic__MAX']
...
# plt.tight_layout()
pp.savefig(bbox_inches='tight', pad_inches=.1)
