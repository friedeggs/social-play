...
class RewardModel(nn.Module):
    def __init__(self):
        ...
        self.optimizer = optim.Adam(list(self.cnn.parameters()) + list(self.linear.parameters()), lr=3e-5, eps=1e-5)
    ...
    def generate_test_set(self, env_class, log_dir):
        np.random.seed(seed)
        self.test_data = []
        for i in range(1000):
            agent_goal_idx = np.random.choice(25*24)
            goal_idx = agent_goal_idx % 25
            agent_idx = agent_goal_idx // 25
            ...
            eval_env = env_class(
                n_walls=3 if env_class == ComplexEnv else 3,
                goal_pos=goal_pos, 
            ...
            state_action = self.encode_action(state, action)
            self.test_data.append((state_action, action, reward))
    ...
    def filter_train(self):
        ...
        for i in tqdm(range(len(self.data))):
            state_action, action, reward = self.data[i]
            exists = False
            for j in range(len(self.test_data)):
                state_action2, action2, reward2 = self.test_data[j]
                if (state_action == state_action2).all():
                    exists = True
                    break
            if not exists:
                new_data.append((state_action, action, reward))
        print(f'Filtered out {len(self.data)-len(new_data)} datapoints')
        self.data = new_data
    ...
    def learn(self, n_steps=1e4):
        ...
        num_data = len(self.test_data)
        if num_reward == 0:
            test_weights = [1./num_data for _ in range(num_data)]
        else:
            frac_rew = 1./(2*num_reward)
            frac = 1./(2*(num_data-num_reward))
            test_weights = [frac_rew if reward > 0. else frac for _, _, reward in self.test_data]
        ...
        os.makedirs("log_reward_model", exist_ok=True)
        ...
        for epoch_id in range(num_epochs):
            for i, batch in tqdm(enumerate(train_dataloader)):
                ...
                # if epoch_id * len(train_dataloader) + i >= n_steps:
                #     break
                ...
                # if epoch_id * len(train_dataloader) + i % 1000 == 0:
                loss_test = self.eval_test(n_sample=1)
                writer.add_scalar('Loss/test', loss_test, epoch_id * len(train_dataloader) + i)
    ...
    def eval_test(self, n_sample=None):
        loss_tot = 0.
        # data = np.random.permutation(self.test_data)
        do_sample = n_sample is not None
        n_sample = n_sample or len(self.test_data)
        # for i, batch in enumerate(self.test_dataloader):
        for i in range(n_sample):
            if do_sample:
                j = np.random.choice(range(len(self.test_data)), p=self.test_weights)
            else:
                j = i
            state_action, action, reward = self.test_data[j]
            # state_action, action, reward = batch
            # pred, loss = self.forward(state_action, reward)
            pred, loss = self.forward(state_action[None], torch.Tensor([reward]))
            loss_tot += loss.item()
        if not do_sample:
            print(f'Test loss: {loss_tot / n_sample}')
        # print(len(self.test_data))
        # import pdb; pdb.set_trace()
        return loss_tot / n_sample

    def eval_weird(self):
        loss_tot = 0.
        for i in range(len(self.weird_data)):
            obs, gt = self.weird_data[i]
            pred, loss = self.forward(obs, gt)
            loss_tot += loss
        print(f'Weird loss: {loss_tot / len(self.weird_data)}')
        # import pdb; pdb.set_trace()
...
class SaveOnBestTrainingRewardCallback(BaseCallback):
    ...
    :param verbose: (int)
    """
    def __init__(self, num_steps: int, check_freq: int, log_dir: str, log_interval, verbose=1):
...
def main():
    log_dir = "./ppo_minigrid_figure_3b/"
    ...
    # if not os.path.exists('reward_model_data-fixed.pkl'):
    ...
    model.learn(args.n_data, callback=callback, progress_bar=True, tb_log_name="single_run", log_interval=None) # 2e5

    #     pickle.dump(reward_model.data, open('reward_model_data-fixed.pkl', 'wb'))
    # else:
    #     reward_model.data = pickle.load(open('reward_model_data-fixed.pkl', 'rb'))

    # reward_model.generate_test_set(SimpleEnv, log_dir)
    # reward_model.filter_train()

    # reward_model.learn() # //10)
    reward_model.learn(n_steps=args.n_data)# 1 5 40 80 160 320#1e4,2e4,3e4,4e4,8e4,1.2e5

    test_loss = reward_model.eval_test()

    with open(f'results-3b-{args.seed}-{args.n_data}.txt', 'w') as f:
        f.write(str(test_loss))

    # reward_model.eval_weird()
    ...
    # # model = PPO.load(callback.save_path)
    # model.env = eval_env
    # model.learn(1e4, callback=callback, progress_bar=True, tb_log_name="single_run", reset_num_timesteps=False)
