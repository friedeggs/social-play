...
class RewardModel(nn.Module):
    ...
    def evaluate_enumerate(self, env_class, log_dir, accuracy, filename):
        ...
        #         for agent_dir in range(4):
                    ...
                    eval_env = env_class(
                        n_walls=5 if env_class == ComplexEnv else 1,
                        goal_pos=goal_pos, 
...
def main():
    ...
    log_dir = f"./ppo_minigrid_figure_4b-walls-{args.mode}-{args.seed}/"
    save_path = os.path.join(log_dir, f"figure_4b-walls-{args.mode}-{args.seed}")
    ...
    callback = SaveOnBestTrainingRewardCallback(1e3, check_freq=1e3, log_dir=log_dir, log_interval=None)
    ...
    # eval_env = ComplexEnv(n_walls=5, render_mode="rgb_array", reward_model=reward_model)
    # eval_env = ComplexEnv(n_walls=5, render_mode="rgb_array")
    eval_env = SimpleEnv(n_walls=1, render_mode="rgb_array")
    ...
    print(f'{mean_reward}\t{std_reward}\t{log_dir}')
    env = eval_env.unwrapped
    print(f'{env.reset_counter / 100}')
