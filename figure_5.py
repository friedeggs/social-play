...
def parse_args():
    ...
    parser.add_argument("--mode", type=str, default='reward-model', # options: ['reward-model', 'oracle', 'frozen']
        help="the name of this experiment")
    parser.add_argument("--seed", type=int, default=1234,
        help="seed of the experiment")
    parser.add_argument("--env_id", type=int, # 1 to 5
        help="seed of the experiment")
...
class RewardModel(nn.Module):
    ...
    def evaluate_enumerate(self, env_class, log_dir, accuracy, filename):
        ...
        #         for agent_dir in range(4):
                    ...
                    # # map_idx 4
                    # if agent_pos in [(3, 1),(1, 3),(3, 3),(5, 3),(3, 5)]: continue
                    # if goal_pos in [(3, 1),(1, 3),(3, 3),(5, 3),(3, 5)]: continue

                    # # map_idx 3
                    # if agent_pos in [(2, 2),(3, 2),(4, 2),(3, 3),(3, 4)]: continue
                    # if goal_pos in [(2, 2),(3, 2),(4, 2),(3, 3),(3, 4)]: continue

                    # # map_idx 2
                    # if agent_pos in [(2, 2),(2, 3),(3, 4),(4, 3),(4, 2)]: continue
                    # if goal_pos in [(2, 2),(2, 3),(3, 4),(4, 3),(4, 2)]: continue

                    # # map_idx 1
                    # if agent_pos in [(2, 2),(2, 4),(3, 3),(4, 2),(4, 4)]: continue
                    # if goal_pos in [(2, 2),(2, 4),(3, 3),(4, 2),(4, 4)]: continue

                    eval_env = env_class(
                        n_walls=5 if env_class == ComplexEnv else 3,
                        goal_pos=goal_pos, 
                    ...
                    # for action in [2]:
                        ...
                        # print(reward, pred, action)

                        if int(reward) == 1.:
                            matrix[int(reward)][min(1, round(pred))] += pred
                        else:
                            matrix[int(reward)][min(1, round(pred))] += 1-pred

                        counts[int(reward)][min(1, round(pred))] += 1
...
class ComplexEnv(MiniGridEnv):
    ...
    def _gen_grid(self, width, height):
        ...
        if self.handpicked:
            idx = self.map_idx if self.map_idx is not None else np.random.choice(5)
            if idx == 0:
    ...
    def _reward(self) -> float:
        ...
        if self.reward_mode == 'default':
            ...
            reward = self.reward_model(state_action[None])[0]
            # reward = 0.4 * torch.round(reward * 2.5)
...
class SaveOnBestTrainingRewardCallback(BaseCallback):
    ...
    def __init__(self, num_steps: int, check_freq: int, log_dir: str, log_interval: int, verbose=1):
        ...
        # eval_env = SimpleEnv(n_walls=1, render_mode="rgb_array")
    ...
    def _on_step(self) -> bool:
        ...
        if self.log_interval is not None and self.n_calls % self.log_interval == 0:
            ...
            # import pdb; pdb.set_trace()
            self.model.logger.record("rollout/ep_rew_mean_extrinsic", 0.4 * env.reset_counter / sum([1 for ep_info in self.model.ep_info_buffer]))
            env.reset_counter = 0
        ...
        if self.check_freq is not None and self.n_calls % self.check_freq == 0:
...
def main():
    ...
    # log_dir = f"./ppo_minigrid_figure_5-walls-5-reward-model-{seed}-0-12e4/"
    # save_path = os.path.join(log_dir, f"figure_5-walls-5-reward-model-{seed}-0-12e4")
    log_dir = f"./ppo_minigrid_figure_5-replicate-walls-handpicked-{args.mode}_env-{args.env_id}-{args.seed}/"
    save_path = os.path.join(log_dir, f"figure_5-replicate-walls-handpicked-{args.mode}_env-{args.env_id}-{args.seed}")
    ...
    # env = ComplexEnv(render_mode="human", handpicked=True, map_idx=args.env_id-1)
    ...
    callback = SaveOnBestTrainingRewardCallback(1e3, check_freq=1e4, log_dir=log_dir, log_interval=1e3)

    # if not os.path.exists('reward_model_data-figure-5_walls-3.pkl'):
        
    if args.mode == 'reward-model':
        env = SimpleEnv(n_walls=1, render_mode="rgb_array", reward_model=reward_model)
    else:
        env = SimpleEnv(n_walls=1, render_mode="rgb_array")
    # env = ComplexEnv(render_mode="rgb_array")
    env = FullyObsWrapper(env)
    env = Monitor(env, log_dir)
    ...
    model.learn(12e4, callback=callback, progress_bar=True, tb_log_name="single_run")#, log_interval=1e3) # 2e5
    # model.save(callback.save_path)

    # pickle.dump(reward_model.data, open('reward_model_data-figure-5_walls-3.pkl', 'wb'))
    # else:
    #     reward_model.data = pickle.load(open('reward_model_data-figure-5_walls-3.pkl', 'rb'))

    # reward_model.learn(n_steps=0)
    if args.mode == 'reward-model':
        reward_model.learn(n_steps=12e4)
    ...
    # env = ComplexEnv(n_walls=5, render_mode="rgb_array", handpicked=True, map_idx=args.env_id-1)
    # env = FullyObsWrapper(env)
    # env = Monitor(env, log_dir)
    # # acc = reward_model.evaluate(env, model, n_eval_episodes=100)
    # # print(f'Reward Model accuracy on complex environment: {acc.item():.2f}')

    # matrix, counts = reward_model.evaluate_enumerate(ComplexEnv, log_dir, 0., 'accuracy_complex.png')


    if args.mode == 'reward-model':
        eval_env = ComplexEnv(n_walls=5, render_mode="rgb_array", reward_model=reward_model, handpicked=True, map_idx=args.env_id-1)
    elif args.mode == 'oracle':
        eval_env = ComplexEnv(n_walls=5, render_mode="rgb_array", handpicked=True, map_idx=args.env_id-1)
    elif args.mode == 'frozen':
        eval_env = SimpleEnv(n_walls=1, render_mode="rgb_array")
    # eval_env.reward_mode = 'none'
    if args.mode in ['oracle', 'frozen']:
        eval_env.reward_mode = 'default'
    elif args.mode == 'reward-model':
        eval_env.reward_mode = 'reward_model'
    eval_env = FullyObsWrapper(eval_env)
    eval_env = Monitor(eval_env, log_dir)
    eval_env = DummyVecEnv([lambda: eval_env])
    ...
    model = PPO.load(callback.save_path)
    model.env = eval_env
    callback = SaveOnBestTrainingRewardCallback(1e3, check_freq=None, log_dir=log_dir, log_interval=1e3)
    model.learn(24e4, callback=callback, progress_bar=True, tb_log_name="single_run", reset_num_timesteps=False)
    ...
    print(f'FINAL REWARD: {mean_reward}\t{std_reward}\t{log_dir}')


    with open(f'results-5-{args.seed}-{args.mode}-{args.env_id}.txt', 'w') as f:
        f.write(str(mean_reward) + '\t' + str(std_reward))

    # # eval_env = SimpleEnv(n_walls=1, render_mode="human")
    # eval_env = ComplexEnv(n_walls=5, render_mode="human", handpicked=True, map_idx=args.env_id-1)
    # # eval_env.reward_mode = 'reward_model'
    # eval_env = FullyObsWrapper(eval_env)
    # eval_env = Monitor(eval_env, log_dir)
